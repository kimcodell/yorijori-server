...
export default class PostService {
  ...
  public async create(params: WritingPostDto) {
    const walletAddress = await this.userRepository.getWalletAddress(
      params.userId,
    );
    if (!walletAddress) {
      throw new ErrorWithCode(
        "WALLET ADDRESS IS REQUIRED",
        "등록된 지갑 주소가 없습니다. 지갑을 등록한 후 게시글을 작성해 주세요.",
      );
    }
    await this._checkValidDate({
      startDate: params.certificationStartDate,
      endDate: params.certificationEndDate,
      cycle: params.certificationCycle,
    });
    await this._checkTokenAmount({
      userId: params.userId,
      tokenAmount: params.distributionTokenAmount,
    });

    const newRecipe = await Recipe.create(params);
    return newRecipe;
  }
  ...
  public async update(params: {
    ...
    if (!walletAddress) {
      throw new ErrorWithCode(
        "WALLET ADDRESS IS REQUIRED",
        "등록된 지갑 주소가 없습니다. 지갑을 등록한 후 게시글을 수정해 주세요.",
      );
    }
    if (distributionTokenAmount) {
      await this._checkTokenAmount({
        userId,
        tokenAmount: distributionTokenAmount,
      });
    }
    ...
    if (!isEditable) {
      throw new ErrorWithCode(
        "NOT EDITABLE POST",
        "목표 달성이 진행 중이고 응원 댓글이 없는 게시글만 수정할 수 있습니다.",
      );
    }
    const data = _.omitBy(_.omit(params, ["userId", "id"]), _.isNil);
    await Recipe.update(data, { where: { id, deletedAt: null } });
  }
  ...
  public async delete(params: { userId: number; postId: number }) {
    ...
    if (!isEditable) {
      throw new ErrorWithCode(
        "NOT EDITABLE POST",
        "목표 달성이 진행 중이고 응원 댓글이 없는 게시글만 삭제할 수 있습니다.",
      );
    }
    await Recipe.destroy({ where: { id: params.postId, deletedAt: null } });
  }
  ...
  private async _checkTokenAmount(params: {
    userId: number;
    tokenAmount: string;
  }) {}

  private async _checkValidDate(params: {
  ...
  private async _isEditable(params: { postId: number }) {
    ...
    const comment = await Review.findOne({
      where: { recipeId: params.postId, deletedAt: null },
      attributes: ["id"],
    });
